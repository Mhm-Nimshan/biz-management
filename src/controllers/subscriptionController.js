...
const { executeTenantQuery } = require('../config/tenantDbConnection');
  ...
  try {
    ...
    const [tenantResult] = await connection.execute(
      `INSERT INTO tenants (tenant_name, tenant_slug, database_name, company_name, email, phone, address, city, state, country, owner_name, status, trial_ends_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'trial', ?)`,
      [tenant_name, uniqueSlug, `nimsleas_${uniqueSlug}`, company_name || tenant_name, email, phone, address, city, state, country, owner_name, trialEndsAt]
    );
...
// Tenant Login (supports tenant_users and system users tables)
exports.tenantLogin = async (req, res) => {
  try {
    const { email, password } = req.body;

    // First try: tenant_users (by email or full_name)
    ...
    if (tenantUsers.length > 0) {
      ...
      await db.execute('UPDATE tenant_users SET last_login = NOW() WHERE id = ?', [user.id]);
      ...
      return res.json({
        ...
        user: {
          ...
          is_tenant_user: true, // Flag to identify this is from tenant_users table (main DB)
          tenant_user_id: user.id // Store tenant_users.id separately
        }
      });
    }

    // Fallback: check system users table (users) in tenant databases
    // Get all active tenants with their database names
    const [tenants] = await db.execute(
      'SELECT id, tenant_slug, tenant_name, database_name, status FROM tenants WHERE status IN ("active", "trial")'
    );

    // Try to find user in any tenant's users table
    let sysUser = null;
    ...
    for (const tenant of tenants) {
      try {
        ...
        if (users.length > 0) {
          ...
          // Treat NULL status as active; block only explicit inactive/suspended
          const userStatus = (candidate.status || 'active').toLowerCase();
          const isBlocked = userStatus === 'inactive' || userStatus === 'suspended';
        ...
        // Database might not exist or table might not be created yet, continue to next tenant
        console.log(`Skipping tenant ${tenant.tenant_slug} - ${error.message}`);
        continue;
      }
    }

    if (!sysUser || !foundTenant) {
      return res.status(401).json({ error: 'Invalid credentials' });
    }

    // Update last login
    await executeTenantQuery(
      foundTenant.database_name,
    ...
    const token = jwt.sign(
      { 
        ...
        tenantSlug: foundTenant.tenant_slug
      },
      process.env.JWT_SECRET || 'your-secret-key',
      { expiresIn: '7d' }
    );
    ...
    return res.json({
      ...
      user: {
        id: sysUser.id,
        email: sysUser.email,
        full_name: sysUser.username,
        role: sysUser.role,
        tenant_id: foundTenant.id,
        tenant_slug: foundTenant.tenant_slug,
        tenant_name: foundTenant.tenant_name,
        status: foundTenant.status === 'trial' ? 'trial' : 'active',
        is_tenant_user: false, // Flag to identify this is from users table (tenant DB)
        user_id: sysUser.id // Store users.id from tenant database
      }
    });
  } catch (error) {
