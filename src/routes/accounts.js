...
router.get('/', async (req, res) => {
  try {
    const [accounts] = await req.db.execute('SELECT * FROM accounts ORDER BY account_name');
    res.json(accounts);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    const [accounts] = await req.db.execute('SELECT * FROM accounts WHERE id = ?', [req.params.id]);
