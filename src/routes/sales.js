...
router.get('/', async (req, res) => {
  try {
    const [sales] = await req.db.execute(`
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [saleResult] = await req.db.execute(
      `INSERT INTO sales 
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    const [saleItems] = await req.db.execute(
    ...
    // Delete sale items
    await req.db.execute('DELETE FROM sale_items WHERE sale_id = ?', [req.params.id]);

    // Delete sale
    const [result] = await req.db.execute('DELETE FROM sales WHERE id = ?', [req.params.id]);
